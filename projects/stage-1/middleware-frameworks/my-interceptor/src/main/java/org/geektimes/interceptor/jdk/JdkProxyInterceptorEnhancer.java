 ...
 * @since
 */
public class JdkProxyInterceptorEnhancer implements InterceptorEnhancer {

    @Override
    public <T> T enhance(T source, Class<? super T> type, Object... interceptors) {
        ClassLoader classLoader = getClassLoader(type);
        return (T) Proxy.newProxyInstance(classLoader, new Class[]{type}, new InvocationHandlerAdapter(source, interceptors));
    }
}
